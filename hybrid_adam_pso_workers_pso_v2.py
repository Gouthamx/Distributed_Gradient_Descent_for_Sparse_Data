...
# ---------------------------------------------------------------------------
# Data loading: reuse splits.pt from Approach_1
# ---------------------------------------------------------------------------
...
def load_tensor_splits(max_samples_per_split: int | None = MAX_SAMPLES_PER_SPLIT):
    ...
    # IMPORTANT:
    # Compute global embedding sizes from the FULL data (train + test) BEFORE any subsampling.
    # Otherwise, subsampling can drop the max user/movie ids and cause embedding IndexError in workers.
...
# ---------------------------------------------------------------------------
# PSO helper over a scalar alpha mixing global and perturbed global
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Worker entry point: PSO only (no Adam in workers)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Main orchestration: PSO in workers, Adam in main, with time limits
# ---------------------------------------------------------------------------
...
def run_hybrid_adam_pso_workers_pso(
    ...
    print("\n[Main] =========================================")
    print("[Main] Hybrid v2: Adam first, PSO in workers (minimal comm: alpha+seed only)")
    print(
    ...
    for r in range(num_rounds):
        ...
        # --- Step 1: Global Adam on current global model (FIRST) ---
        ...
        for ge in range(global_adam_epochs):
            train_loss = train_one_epoch_adam(global_model, global_train_loader, optimizer, loss_fn)
            test_loss = evaluate_model(global_model, global_test_loader, loss_fn)
            print(f"[Main] Global Adam epoch {ge+1}/{global_adam_epochs} | train_loss={train_loss:.6f} | test_loss={test_loss:.6f}")

        # --- Step 2: Snapshot post-Adam model and dispatch PSO workers (SECOND) ---
        ...
        for _ in range(num_workers):
            ...
            if not msg.get("ok", True):
                ...
                # Mark as a dead round; we'll abort after cleanup
                any_dead = True
                continue

            worker_test_losses.append(msg["local_test_loss"])
            # Reconstruct worker state from alpha+seed (minimal comm: no full model)
        ...
        # Load aggregated PSO result as the new global model (no Adam here; Adam runs at round start)
    ...
    # Communication: main->workers = 1 full model broadcast; workers->main = alpha+seed only (minimal)
    ...
    comm_stats = {
        ...
        "payload_mode": "minimal",  # workers send alpha+seed, not full model
...
def main():
    ...
    # Save results for plotting (minimal: state_dict, history, comm_stats)
    out_path = "hybrid_adam_pso_workers_pso_v2_results.pt"
    torch.save(
        {
